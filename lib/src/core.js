    ...
    const processNode = (node) => {
        switch (node.kind) {
            ...
            case ts.SyntaxKind.EnumDeclaration:
                ...
                if (traverseProperty) {
                    if (property === traverseProperty || (traverseProperty.split('.').length > 1 && traverseProperty.split('.')[0] === property)) {
...
function processEnum(node, sourceFile, options, output, traverseProperty) {
    if (traverseProperty.split('.').length > 1) {
        ...
        if (targetMember?.initializer && targetMember.initializer.kind === ts.SyntaxKind.StringLiteral) {
            basicReferenceValue = faker_1.default.fake('{{lorem.text}}').substring(0, 50);
        }
        else {
            basicReferenceValue = parseInt(faker_1.default.fake('{{datatype.number}}'), 10);
        ...
        node.members.find(element => {
            if (element.initializer && element.initializer.kind === ts.SyntaxKind.StringLiteral) {
                values.push(faker_1.default.fake('{{lorem.text}}').substring(0, 50));
                return true;
            }
        });
        node.members.find(element => {
            if (!element.initializer || (element.initializer && element.initializer.kind === ts.SyntaxKind.NumericLiteral)) {
